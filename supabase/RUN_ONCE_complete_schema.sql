-- ============================================================================
-- Run this ENTIRE file once in Supabase → SQL Editor (new / empty project).
-- Do not paste the filename — only the SQL below.
-- After this, your Node app can use service role + .env SUPABASE_* keys.
-- ============================================================================

CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Workspaces (id 1 = default hub)
...
-- Backfill message workspace from sender (no-op if no rows)
UPDATE messages m
SET workspace_id = u.workspace_id
FROM users u
WHERE m.sender_id = u.id AND m.workspace_id IS NULL;

-- Default team + #general (required for dashboard channel list)
